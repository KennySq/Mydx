...
namespace Mydx
{
	const Mesh& Mydx::gen::GenerateSphere(float radius, unsigned int sliceCount, unsigned int stackCount)
	{
	...
	bool Mydx::gen::GenerateMeshBuffer(std::vector<_VertTy>& vertices, std::vector<UINT>& indices, Mesh& const buffer)
	{
		bool result = false;
		ID3D11Device* device = HW::GetDevice();

		unsigned int vertCount = vertices.size();
		unsigned int indCount = indices.size();

		D3D11_BUFFER_DESC vDesc{}, iDesc{};
		D3D11_SUBRESOURCE_DATA vSub{}, iSub{};

		vDesc.ByteWidth = sizeof(_VertTy) * vertCount;
		vDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
		vDesc.Usage = D3D11_USAGE_DEFAULT;

		vSub.pSysMem = vertices.data();

		result = device->CreateBuffer(&vDesc, vSub,
